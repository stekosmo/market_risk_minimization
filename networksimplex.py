...
def network_simplex(G, demand='demand', capacity='capacity', weight='weight'):
    ...
    #Total demand calculation now incorperates a tolerance to work with floats
    if abs(sum(D)) > 10**(-2):
    ...
    #Flow calculation now incorperates a tolerance to work with floats
    if any(abs(x[i]) > 10**(-2) for i in range(-n, 0)):
